    ...
    <div class="row">
        ...
        <div class="form-group col-xs-12">
            <div class="input-group">
                ...
                <input type="file" class="form-control image-file" id="image" aria-describedby="image" placeholder="Subir una imagen..." accept="image/*">          
                <span class="up-label">Subir una imagen...</span>
            </div>
        </div>
        <div class="form-group col-xs-12">
            <div class="input-group">
                <button type="button" class="btn btn-primary" onclick="save(this)" id="save">Guardar</button>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    var openUpdate = function(quotID){
        $('.tag').remove();
        $('#topic').val('');
        var quote = find_by('quotes_es','quoteID',quotID);
        ...
        quote.done(function(data){
            if(Object.keys(data[0][0]).length > 1){
                ...
                var getTopics = find_by('quotesTopicES','quoteID',quotID);
                getTopics.done(function(rel){
                    for(var i in rel[0]){
                        var topic = find_by('topics_es','topicID',rel[0][i].topicID);
    ...
    var updateQuote = function(el, quotID){
        ...
        if(!quote || quote==''){
            ...
            token.done(function(generatedToken){
                var quoteUpdate = update('quotes_es',arr,'quoteID',quotID,generatedToken);
                quoteUpdate.done(function(data){
                    console.log(data);
                    var token2 = generateToken();
                    token2.done(function(generatedToken2){
                        var deleteRel = delete_function('quotesTopicES','quoteID',quotID,generatedToken2);
                                ...
                                topicSearch.done(function(topicId){
                                    ...
                                    token3.done(function(generatedToken3){
                                        ...
                                        topicQuote.done(function(data2){
                                            ...
                                            el.innerHTML = "Actualizar!";
                                            setTimeout(function() {
                                                spanish('Quote saved successfully!',document.getElementById('es'));
                                            }, 1000);
                                        });
                                    });
                                });
